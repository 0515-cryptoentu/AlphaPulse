...
def fetch_transactions(wallet):
    url = f"https://public-api.solscan.io/account/transactions?account={wallet}&limit=25"
    try:
...
def analyze_wallet(wallet):
    ...
    recent_txs = [
        tx for tx in txs
        if "blockTime" in tx and tx["blockTime"] > (time.time() - TRADE_LOOKBACK_HOURS * 3600)
    ]
    ...
    score = (len(recent_txs) * 0.5 + len(tokens) * 0.4 + (1 if last_active > time.time() - 86400 else 0) * 0.1) * 10
...
def store_wallets(data_list):
    ...
    for w in data_list:
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (w['wallet'], w['tx_count'], w['token_count'], w['last_active'], w['score']))
    conn.commit()
    conn.close()
