...
auth = tweepy.OAuth1UserHandler(consumer_key=API_KEY_TWITTER, consumer_secret=API_SECRET_KEY_TWITTER, 
                                access_token=ACCESS_TOKEN_TWITTER, access_token_secret=ACCESS_TOKEN_SECRET_TWITTER)
api = tweepy.API(auth)

# Targeted crypto Twitter accounts
CRYPTO_ACCOUNTS = [
    "JacobCryptoBury", "MarkKellyCrypto", "Bluntz", "Pentoshi", "CrediBullCrypto",
    "DonAlt", "CryptoCred", "CryptoChase", "ColdBloodedShiller", "NinjaScalp", "CryptoLouca"
]
...
def get_tx_metrics(wallet):
    try:
        ...
        timestamps = [sig.get("blockTime") for sig in signatures if sig.get("blockTime")]
        if len(timestamps) < 2:
            return 0, -1
...
def add_wallet(wallets, wallet, source):
    if re.match(r"^[1-9A-HJ-NP-Za-km-z]{32,44}$", wallet):
        ...
        if tx_count >= MIN_TX_COUNT and (avg < MAX_AVG_INTERVAL and avg > 0):
            wallets.append({"wallet": wallet, "source": source, "tx_count": tx_count, "avg_interval": avg})
...
if __name__ == "__main__":
    wallet_list = ["9djU9o4CD14ak5G4TNLp1KvqbWZ4BptU6WyquvDjWYJz", "FjYFNY2KXwRbDMEiEdxAF3uCWJTL5sCHwMGR6ZaSkbtu"]  # Sample wallets
